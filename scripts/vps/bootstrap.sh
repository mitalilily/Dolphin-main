...
if [ ! -f "$BACKEND_ENV_SOURCE" ]; then
  ...
  CLIENT_EXTRA_ORIGINS=""
  for domain in $CLIENT_EXTRA_DOMAINS; do
    CLIENT_EXTRA_ORIGINS="${CLIENT_EXTRA_ORIGINS},https://${domain}"
  done
  CORS_ORIGIN_LIST="${CORS_ORIGIN_LIST:-${PUBLIC_ORIGIN},${CLIENT_ORIGIN}${CLIENT_EXTRA_ORIGINS},${ADMIN_ORIGIN}${DOMAIN_ORIGINS}}"
...
set_env_value "$BACKEND_ENV_SOURCE" "RAZORPAY_MODE" "live"
...
PUBLIC_ORIGIN="$PUBLIC_ORIGIN" CLIENT_ORIGIN="$CLIENT_ORIGIN" CLIENT_DOMAIN="$CLIENT_DOMAIN" CLIENT_EXTRA_DOMAINS="$CLIENT_EXTRA_DOMAINS" ADMIN_ORIGIN="$ADMIN_ORIGIN" API_ORIGIN="$API_ORIGIN" API_DOMAIN="$API_DOMAIN" bash "$APP_DIR/scripts/vps/deploy.sh"
